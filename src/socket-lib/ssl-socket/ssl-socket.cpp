...
SslSocket::~SslSocket() { this->closeConnection(); }
...
void SslSocket::sendAll(const std::string &data) {
  ...
  while (totalSent < data.size()) {
    int sent =
        SSL_write(ssl, data.c_str() + totalSent, data.size() - totalSent);
    if (sent <= 0) {
...
std::string SslSocket::receiveAll() {
  ...
  while (true) {
    ...
    // when got data then append
    if (bytesRead > 0) {
      result.append(buffer, bytesRead);
    }
    // when end of data then end loop
    else if (bytesRead == 0) {
      break;
    }
    // when error happened then handle
    else {
      int err = SSL_get_error(ssl, bytesRead);

      std::cerr << err << " " << bytesRead << std::endl;
