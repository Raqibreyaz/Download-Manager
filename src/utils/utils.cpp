...
getFilenameAndExtension(const std::string &contentDisposition,
...
void saveToFile(const std::string &filename, const std::string &data) {
  // std::clog << "[DEBUG] Trying to open file: '" << filename << "'" <<
  // std::endl;
  ...
  if (!file) {
    ...
    std::clog << "[DEBUG] Parent directory: '" << dir.string() << "'"
              << std::endl;
    std::clog << "[DEBUG] Directory exists: "
              << (std::filesystem::exists(dir) ? "YES" : "NO") << std::endl;
